...
}
{
  ...
  % (for example, as we had to do with the \texttt{background} package, included in the distribution in the \texttt{sty/background} directory). The easiest way is to install \texttt{texlive-full}.
  ...
  \begin{itemize}
    \item On Linux
          ...
          \begin{itemize}
            ...
            % \item \texttt{latexdiff} and \texttt{latexpand}, if you want to use the feature for generating PDF files with change tracking. The former is usually a separate package, and the latter is usually found in the \texttt{texlive-extra-utils} package.
          \end{itemize}


    \item On Windows (I'll provide the configuration I've tested at some point):

          \begin{itemize}
            \item \texttt{miktex} as the \LaTeX{} compilation system, instructing it to install everything needed.
            \item \texttt{TeXstudio} or \texttt{TeXMaker} as the editor.
            \item To use glossaries, take a look at \url{https://tex.stackexchange.com/questions/156270/has-anyone-managed-to-use-glossaries-with-texstudio-on-windows}. Here's the information we received (thanks, Cristina Losada): In Texmaker, you can configure the "Quick Build" with the commands you want. That


            \item To use glossaries, take a look at \url{https://tex.stackexchange.com/questions/156270/has-anyone-managed-to-use-glossaries-with-texstudio-on-windows}. Here's the information we received (thanks, Cristina Losada): In Texmaker, you can configure the "Quick Build" with the commands you want. Go to the Options menu, select Configure Texmaker, and on the left side of the window that appears, choose "Quick Build". There are typical options available, and it also allows using user-defined commands. Here's my configuration (it compiles with pdflatex, also calls bibtex and makeglossaries, and finally displays the PDF): \texttt{pdflatex -synctex=1 -interaction=nonstopmode %.tex|bibtex %|makeglossaries %|pdflatex -synctex=1 -interaction=nonstopmode %.tex|pdflatex -synctex=1 -interaction=nonstopmode %.tex|}''\texttt{C:/Program Files (x86)/Adobe/Acrobat Reader DC/Reader/AcroRd32.exe}''\texttt{ %.pdf}
            \item \texttt{make}, if you want to use the convenience of the supplied \texttt{Makefile}. There are several options and it requires a command interpreter (it may be even easier with the new WSL).
          \end{itemize}
  \end{itemize}
